...
def get_pet_labels(image_dir):
    ...
    for filename in in_files :
      ...
      if in_files[0] != ".":
         pet_label.append(fname)

    for idx in range(0, len(in_files), 1):
      if in_files[idx] not in results_dic:
        results_dic[in_files[idx]] = [pet_label[idx]]
      else:
        print("** Warning: Duplicate files exist in directory: the key =" ,in_files[idx],
        "its already exist in result dictionary and its value=",results_dic[in_files[idx]])
